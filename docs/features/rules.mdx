...
<Frame caption="Rockxy rule-management surfaces for traffic modification rules.">
...
Rockxy currently exposes rule editing through dedicated tool windows rather than a single generic Rules editor. Open the window for the action you want, then create a rule from scratch or quick-create one from a captured request or sidebar domain where supported.

Typical flow:

1. Open the relevant tool window from **Tools**.
2. Create a new rule or use a request/domain quick-create action.
3. Configure the match condition — URL pattern, HTTP method, or header match when the editor supports it.
4. Fill in the action-specific configuration (file path, target URL, delay, header operations, etc.).
5. Save the rule. It takes effect for new matching requests.
...
Rules are stored as JSON at `~/Library/Application Support/com.amunx.rockxy.community/rules.json` for the Community build. This file is human-readable and version-control friendly.

**Import/Export:**

- **Export** — use the rule window's More menu or context menu when available. Exports save a `.json` file you can share.
- **Import** — use the matching rule window's import action when available. Imported rules are appended to the existing list after validation.
...
- **Reorder** where the dedicated rule window exposes ordering controls.
