...
Grab the latest release from [GitHub Releases](https://github.com/RockxyApp/Rockxy/releases). Rockxy currently ships a universal `.dmg` that supports both Apple Silicon and Intel Macs.
...
1. In the Rockxy menu bar, go to **Certificate > Install on This Mac...**.
2. Enter your macOS password if prompted to add and trust the certificate.
...
Switch to the **Logs** tab in the main content switcher to see captured application logs from OSLog streams and process stdout/stderr. Rockxy currently correlates logs to nearby requests by timestamp so you can jump between network traffic and related log output while debugging.
...
<CardGroup cols={2}>
  ...
  <Card title="Keyboard Shortcuts" icon="keyboard" href="/customization/keyboard-shortcuts">
    Current menu shortcuts for proxy control, sessions, tabs, rules, diff, and scripting.
  </Card>
