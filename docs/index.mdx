...
Select any captured request and re-send it with **Repeat** (fast path) or **Edit and Repeat** (edit method, URL, headers, and body first). Replay uses a proxy-bypass session, so replayed traffic does not re-enter the proxy pipeline. The response is shown in the compose/replay surface with status, headers, and body.
