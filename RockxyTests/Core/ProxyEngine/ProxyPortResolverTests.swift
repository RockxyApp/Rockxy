...
struct ProxyPortResolverTests {
    ...
    @Test("resolve with listenIPv6 true still succeeds")
    func resolveWithListenIPv6DoesNotBreak() throws {
        let freePort = try findFreePort()
        let resolution = try ProxyPortResolver.resolve(
            preferred: freePort,
            address: "127.0.0.1",
    ...
    @Test("resolve fallback port is in valid TCP port range")
    func fallbackPortIsInValidRange() throws {
        let listener = try TCPListener(port: 0, address: "127.0.0.1")
        defer { listener.close() }
        ...
        #expect(resolution.isFallback == true)
        #expect(resolution.port >= 1)
        #expect(resolution.port <= 65535)
    }

    @Test("error thrown contains the preferred port number")
    func thrownErrorContainsPreferredPort() throws {
        let listener = try TCPListener(port: 0, address: "127.0.0.1")
        defer { listener.close() }
        let occupiedPort = listener.boundPort

        do {
            _ = try ProxyPortResolver.resolve(
                preferred: occupiedPort,
                address: "127.0.0.1",
                autoSelect: false
            )
            Issue.record("Expected error to be thrown")
        } catch let error as ProxyServerError {
            if case .portInUse(let port) = error {
    ...
    @Test("nearby ports are tried before ephemeral range")
    func nearbyRangePrioritizedOverEphemeral() throws {
        // Bind to a specific port, then verify the fallback is in the nearby window
        // (preferred+1 ... preferred+100) and not from the ephemeral range (49152..65535)
        // when a nearby port is available.
        let listener = try TCPListener(port: 0, address: "127.0.0.1")
        defer { listener.close() }
        let occupiedPort = listener.boundPort

        // Only test in a port range where nearby scan is meaningful (not close to 65535)
        guard occupiedPort < 65435 else {
            return
        }
        ...
        #expect(resolution.isFallback == true)
        // We can't guarantee the exact fallback port, but it must not equal the occupied port.
        #expect(resolution.port != occupiedPort)
    }

    @Test("resolve with autoSelect false does not scan for alternatives")
    func noScanWhenAutoSelectDisabled() throws {
        let listener = try TCPListener(port: 0, address: "127.0.0.1")
        defer { listener.close() }
    ...
    @Test("resolve returns isFallback false when preferred port is free regardless of autoSelect")
    func noFallbackWhenPreferredIsFree() throws {
        let freePort = try findFreePort()

        // autoSelect: false — preferred is free, should succeed without scanning
        let resolution = try ProxyPortResolver.resolve(
            preferred: freePort,
            address: "127.0.0.1",
            autoSelect: false
