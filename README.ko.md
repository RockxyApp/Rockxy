...
- WebSocket Protobuf 작업은 Rockxy의 더 풍부한 protocol inspection 방향의 일부로 계속됩니다.
- Public roadmap planning에는 이제 AI traffic, Web3/RPC flows, x402-style payment flows를 위한 protocol-aware debugging과 더 안전한 redacted evidence sharing이 포함됩니다.
...
향후 기능은 공개적으로 추적되며 구현, 테스트, privacy behavior, 문서가 준비된 뒤에만 출시됩니다.
...
블록체인 시대의 network call을 읽기 쉬운 debugging evidence로 바꿉니다. JSON-RPC와 Solana RPC traffic을 검사하고, 관련 call을 flow로 묶고, 일반적인 RPC error를 설명하며, Rockxy를 wallet이나 block explorer로 만들지 않고 selected request를 replay합니다.

`JSON-RPC` · `Solana RPC` · `Wallet Flows` · `RPC Errors` · `Replay Helpers` · `Network Evidence`

### x402 Payment Flow Debugging `곧 출시`
...
### Protocol-Aware Filters & Rules `곧 출시`

Rockxy가 이미 작동하는 곳에서 AI와 Web3 metadata를 사용합니다: filter, badge, optional column, comparison, rule, Developer Setup, local MCP summary.
...
로드맵 방향: 더 깊은 replay/diff/rules/scripting 워크플로, 향상된 WebSocket 및 GraphQL 검사, AI와 Web3/RPC를 위한 protocol-aware debugging, x402-style payment-flow visibility, gRPC/Protobuf와 HTTP/2 및 HTTP/3 지원 탐색.
